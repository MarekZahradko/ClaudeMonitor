...
struct AccountToggleViewTests {
    ...
    @Test func theViewReportsARealSizeSoTheHeaderCanPlaceIt() {
        ...
        view.configure(names: names, selectedIndex: 0)
    ...
    @Test func configureKeepsTheLabelsItWasGiven() {
        let view = AccountToggleView(frame: .zero)
        view.configure(names: names, selectedIndex: 1)
        #expect(view.currentLabels == names)
    }
    ...
    @Test func configureIgnoresASelectionOutsideTheSegments() {
        let view = AccountToggleView(frame: .zero)
        view.configure(names: names, selectedIndex: 0)
        view.configure(names: names, selectedIndex: 7)
        let drawn = AccountTogglePill.image(labels: names, selectedIndex: 7)
        #expect(pixelCount(in: drawn, matching: ClaudeGlyph.color) == 0)
        #expect(view.currentLabels == names)
    }
}
