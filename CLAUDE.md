...
All formatting, model, data coordination, rendering logic, and menu-building logic is tested. Services are not unit-tested (they hit real APIs). Window UI mostly is not, with `PreferencesWindowControllerTests` as the exception — it drives the controller with an injected `defaults:`.
